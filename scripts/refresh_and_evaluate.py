...
def evaluate_all(storage="files"):
    print("[market-refresh] loading candles for evaluation",flush=True)
    if storage=="database":
    ...
    print("[market-refresh] evaluating deterministic breakout shadow",flush=True); breakout_config,breakout_hash=load_breakout_shadow_config(Path("model_artifacts/breakout_shadow_config.json")); breakout=evaluate_breakout_shadow(build_strategy_frame(hourly),breakout_config,breakout_hash); save_breakout_shadow_state(breakout,Path("data/breakout_shadow_state.json"))
    print("[market-refresh] evaluating long-trend shadow",flush=True); study_frame=build_study_frame(hourly,m15,daily); trend_config,trend_hash=load_shadow_config(Path("model_artifacts/forward_shadow_config.json")); trend=evaluate_forward_shadow(study_frame,trend_config,trend_hash); save_shadow_state(trend,Path("data/forward_shadow_state.json"))
    print("[market-refresh] evaluating CatBoost breakout shadow",flush=True); ml_config,ml_meta,ml_model=load_ml_shadow(Path("model_artifacts/breakout_ml_shadow_config.json"),Path("model_artifacts/registry.json"),Path("model_artifacts/artifacts")); ml=evaluate_ml_shadow(build_strategy_frame(hourly),breakout_config["atr_pct_low_volatility_cutoff_by_symbol"],ml_config,ml_meta,ml_model); save_ml_shadow(ml,Path("data/breakout_ml_shadow_state.json"))
...
if __name__=="__main__":
    ...
    print(json.dumps(evaluate_all(a.storage),indent=2),flush=True)
    if a.storage=="database":
        from app.database.engine import engine
        engine.dispose()
    print("[market-refresh] completed; exiting",flush=True)
