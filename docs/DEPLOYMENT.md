...
For the optional final AI veto, set `EXTERNAL_AI_REVIEW_ENABLED=true`, add a sealed `OPENAI_API_KEY`, and set `OPENAI_REVIEW_MODEL=gpt-5.6-sol`. This is the quality-first default; any replacement must be explicitly evaluated against the demo decision history before deployment. Missing credentials, API errors, invalid schemas, confidence below `OPENAI_REVIEW_MIN_CONFIDENCE`, and model vetoes all block the signal. The AI is never given a tool or endpoint that can transmit an order.
